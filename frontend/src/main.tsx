...
import { api, ApiError, refreshCsrf, type User, type Catalog } from "./api";
import "./style.css";
function App() {
  ...
  const [resources, setResources] = useState<Catalog[]>([]);
  useEffect(() => {
  ...
  const title = user.role === "Booker" ? "我的预约" : "预约工作台";
  return (
    <div className="shell">
      <aside>
        ...
        <div className="sidebar-bottom">
          ...
          <div>
            <strong>{user.role === "Booker" ? "预约者" : user.role}</strong>
        ...
        <div className="stats">
          ...
          <article>
            ...
            <strong className="text-stat">
              {user.role === "Booker" ? "预约者" : user.role}
            </strong>
            <span>服务端权限保护</span>
          </article>
          <article>
            <small>当前账号</small>
            <strong className="text-stat">{user.name}</strong>
            <span>
              {user.role === "Booker" ? "仅显示本人的预约" : "工作人员工作台"}
            </span>
          </article>
        </div>
        {view === "labs" ? (
