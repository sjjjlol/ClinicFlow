...
test('A21 complete collaborative appointment lifecycle',async({page})=>{
 async function login(role:string){await page.getByRole('combobox',{name:'演示角色'}).selectOption(role);await page.getByLabel('密码',{exact:true}).fill(process.env.DEMO_PASSWORD!);await page.getByRole('button',{name:'进入工作台'}).click();await expect(page.getByRole('heading',{name:'预约工作台',exact:true})).toBeVisible();}
 await page.goto('/');await login('scheduler');await page.getByRole('button',{name:'新建预约'}).click();
 const date=new Date(Date.UTC(2040,0,1)+Math.floor(Math.random()*100000)*900000).toISOString().slice(0,16);
 await page.getByLabel('开始时间（上海 UTC+8）').fill(date);
 const response=page.waitForResponse(r=>r.url().endsWith('/api/appointments')&&r.request().method()==='POST');await page.getByRole('button',{name:'创建预约',exact:true}).click();const a=await (await response).json();
 await page.getByRole('button',{name:'确认预约',exact:true}).click();await expect(page.getByRole('dialog').getByRole('alert')).toContainText('前置任务');
 await page.getByRole('button',{name:'关闭详情'}).click();await page.getByRole('button',{name:'退出登录'}).click();await login('taskoperator');await page.goto('/?appointment='+a.id);
 await page.getByRole('button',{name:'完成 资料核对',exact:true}).click();await page.getByRole('button',{name:'完成 预约信息复核',exact:true}).click();
 await expect(page.getByRole('dialog').getByText('已完成',{exact:true})).toHaveCount(2);
 await expect(page.getByRole('button',{name:'确认预约',exact:true})).toHaveCount(0);
 await page.getByRole('button',{name:'关闭详情'}).click();await page.getByRole('button',{name:'退出登录'}).click();await login('scheduler');
 await page.getByRole('button',{name:'确认预约',exact:true}).click();await expect(page.getByRole('heading',{name:'预约详情'})).toContainText('已确认');
 await page.getByRole('button',{name:'改期',exact:true}).click();await page.getByRole('combobox',{name:'预约资源',exact:true}).selectOption('2');await page.getByRole('button',{name:'保存改期'}).click();
 await expect(page.getByRole('heading',{name:'预约详情'})).toContainText('待确认');await expect(page.getByRole('dialog').getByText('待完成',{exact:true})).toHaveCount(2);
