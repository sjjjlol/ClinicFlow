...
M3 mutations use idempotency → ordered resource parents → appointment row → version/resource revalidation. The initial resource pre-read never grants correctness: if it changed while waiting, return version_conflict and roll back. Reschedule deletes old claims and flushes within the transaction before inserting overlapping new claims. An injected failure after deletion proves rollback restores the old claims, state, version and audit. Two opposite resource moves share the same resource lock order. Cancelled is terminal; identical-key replay returns the prior cancellation, while a new cancellation command is rejected. Query DTOs and replay both emit UTC Z.
