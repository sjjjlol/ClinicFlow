...
# ---- 阶段1：前端构建（React + Vite）----
FROM node:24.13.1-alpine AS frontend   # AS frontend：给阶段命名，后面用 --from=frontend 引用
WORKDIR /src/frontend                  # WORKDIR ≈ cd，后续指令的相对路径基准（不存在则创建）
COPY frontend/package*.json ./         # 层缓存技巧：先只拷依赖清单——package-lock 不变则下一层 RUN 命中缓存
RUN npm ci                             # npm ci 严格按 lock 文件安装（可复现构建；不用 npm install）
COPY frontend/ ./                      # 再拷源码：改源码只重建此层之后的层，依赖层不重跑
RUN npm run build                      # Vite 构建，产物到 dist/

# ---- 阶段2：后端发布（.NET SDK 编译镜像）----
FROM mcr.microsoft.com/dotnet/sdk:10.0.401 AS backend
WORKDIR /src
COPY global.json Directory.Build.props ./   # 构建配置先行：global.json 钉 SDK 版本；Build.props 开 lock 还原
COPY backend/ backend/
RUN dotnet restore backend --locked-mode    # --locked-mode：严格按 packages.lock.json 还原（≈ npm ci）
RUN dotnet publish backend -c Release --no-restore -o /out   # 发布生产产物到 /out（≈ mvn package）
COPY --from=frontend /src/frontend/dist/ /out/wwwroot/       # 前端静态文件塞进 ASP.NET 的 wwwroot
...
EXPOSE 8080                             # 声明监听端口（文档作用；真正映射靠 compose 的 ports）
